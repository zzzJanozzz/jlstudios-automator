// src/templates/gastro-template.ts
...
function waHref(data: BusinessData): string {
  ...
  const m = encodeURIComponent(data.contactInfo.whatsappMessage || "Hola! Quiero hacer un pedido 🍽️");
  return n ? `https://wa.me/${n}?text=${m}` : "#";
}

function fonts(data: BusinessData): string {
  const h = data.designSystem.fontHeading;
  const b = data.designSystem.fontBody;
  const set = [...new Set([h,b])];
  return set.map(f=>`family=${encodeURIComponent(f)}:ital,wght@0,300;0,400;0,500;0,600;0,700;0,800;1,400`).join("&");
}
  ...
  return data.categories.map((cat, i) => {
    ...
    const cards = items.map(it => `
      <div class="menu-card" data-aos>
        ...
        <a href="${waHref(data)}" class="menu-card-order" target="_blank" rel="noopener" aria-label="Pedir ${esc(it.name)}">
...
<style>
/* ══════════════════════════════════════
   GASTRO TEMPLATE — JLStudios Premium
   ══════════════════════════════════════ */
...
.hero-bg{
  ...
  ${hasCover
    ...
    : `background: radial-gradient(ellipse 80% 60% at 50% 30%, rgba(${hexToRgb(ds.primaryColor)},0.35) 0%, transparent 70%),
       linear-gradient(180deg, #0a0a0a 0%, #111 100%);`
  }
}
${hasCover ? `.hero-bg::after{content:'';position:absolute;inset:0;background:linear-gradient(to top, rgba(0,0,0,0.96) 0%, rgba(0,0,0,0.55) 60%, rgba(0,0,0,0.2) 100%)}` : ""}
.hero-content{
...
.scroll-hint{
  ...
  color:rgba(255,255,255,0.3);font-size:0.68rem;letter-spacing:.15em;text-transform:uppercase;
...
.menu-card-body{padding:20px 20px 14px;flex:1}
.menu-card-top{
...
.menu-price{
  ...
  padding:2px 10px;
  background:rgba(${hexToRgb(ds.accentColor)},0.1);
  border-radius:6px;
  border:1px solid rgba(${hexToRgb(ds.accentColor)},0.2);
}
.menu-item-desc{
...
.menu-card-order:hover{
  background:rgba(${hexToRgb(ds.accentColor)},0.15);
...
.footer-tagline{font-size:0.83rem;color:rgba(255,255,255,0.4)}
.footer-col h4{
...
<section class="hero">
  ...
  <div class="hero-content">
    ...
    ${(ci.address || ci.schedule) ? `
    <div class="hero-info">
      ${ci.address ? `<div class="hero-info-chip">
      ...
      ${ci.schedule ? `<div class="hero-info-chip">
...
<nav class="cat-nav" id="menu">
  <div class="cat-nav-inner">
    ${buildCategoryNav(data.categories)}
  </div>
</nav>
...
<!-- FLOATING WA -->
<a href="${wa}" class="wa-float" target="_blank" rel="noopener" aria-label="Pedir por WhatsApp">
...
<footer class="site-footer">
  <div class="footer-inner">
    <div>
      ...
      ${data.tagline ? `<div class="footer-tagline">${esc(data.tagline)}</div>` : ""}
      <div class="footer-social">
        ...
        ${ci.facebook ? `<a href="https://facebook.com/${esc(ci.facebook)}" target="_blank" rel="noopener" class="nav-social" aria-label="Facebook"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg></a>` : ""}
      </div>
    </div>
    ${ci.address || ci.schedule || ci.email ? `
