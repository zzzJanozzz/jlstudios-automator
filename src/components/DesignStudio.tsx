// src/components/DesignStudio.tsx  v2.3
"use client";
...
function ImageUpload({
  label, value, onChange, aspect = "16/5", compact = false,
}: {
  label: string;
  value?: string;
  ...
  const handleFile = useCallback((file: File) => {
    // Resize large images before base64 to avoid bloat
    const MAX_DIM = compact ? 400 : 1400;
  ...
  if (compact) {
    return (
      <div
        className="relative rounded-lg overflow-hidden cursor-pointer border-2 border-dashed flex items-center justify-center transition-colors"
        style={{
      ...
      >
        {value ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
            ...
            <span className="text-xs" style={{ color:"#52525b" }}>Click para subir</span>
          </div>
        )}
        <input ref={ref} type="file" accept="image/*" className="hidden"
          onChange={(e) => { if (e.target.files?.[0]) handleFile(e.target.files[0]); }} />
        ...
        >
          ...
          {activeTab === "identidad" && <>
            ...
            <div>
              ...
              <input type="text" className="input-field" maxLength={80}
                ...
                placeholder="Tu propuesta en menos de 8 palabras"
            ...
            <ImageUpload label="Foto de portada / Hero" value={businessData.portada_b64}
              onChange={(b64) => updateBusinessField("portada_b64", b64)} aspect="16/5" />

            <ImageUpload label="Logo del negocio" value={businessData.logo_b64}
              onChange={(b64) => updateBusinessField("logo_b64", b64)} aspect="1/1" />
          </>}
          ...
          {activeTab === "menu" && <>
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs" style={{ color:"#52525b" }}>{businessData.categories.length} categorías</span>
            </div>
          ...
          {activeTab === "diseno" && <>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  <label className="label-text mb-1 block text-[10px]">Títulos (heading)</label>
                ...
                <div>
                  <label className="label-text mb-1 block text-[10px]">Cuerpo de texto (body)</label>
            ...
            {/* Visual style — affects template CSS */}
            <div>
              <p className="label-text mb-2">Estilo visual</p>
              <p className="text-[10px] mb-3" style={{ color:"#52525b" }}>
                Modifica bordes, sombras, tipografía y espaciado en el sitio generado.
              </p>
                ...
                {STYLE_OPTIONS.map((s) => (
                  <button key={s.value} onClick={() => updateDesignSystem({ style: s.value })}
                    ...
                    <div className="flex-1">
                      <p className="text-xs font-semibold" style={{ color: ds.style === s.value ? "#e4e4e7" : "#a1a1aa" }}>
                        {s.label}
                      </p>
          ...
          {activeTab === "contacto" && <>
            <div className="p-3 rounded-lg text-xs"
              ...
              💡 Todos estos datos son editables desde el CMS del cliente y aparecen visibles en el sitio.
            </div>
              ...
              <div className="space-y-2">
                {ci.schedule.map((day, idx) => (
                  <div key={day.day}
                    ...
                    style={{
                      backgroundColor: day.isOpen ? "rgba(109,40,217,0.06)" : "#18181b",
                      border: `1px solid ${day.isOpen ? "rgba(124,58,237,0.2)" : "#27272a"}`,
                    }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        backgroundColor: day.isOpen ? "#7c3aed" : "#3f3f46",
                      }}
                      aria-label={`${day.isOpen ? "Cerrar" : "Abrir"} ${day.day}`}
                    >
                      <span
                        className="absolute top-0.5 rounded-full bg-white transition-transform"
                        style={{
                          width: "14px", height: "14px",
                          left: day.isOpen ? "15px" : "2px",
